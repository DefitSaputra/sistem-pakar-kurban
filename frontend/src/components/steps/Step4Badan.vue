...
const CODES = ['G09', 'G10', 'G11', 'G12', 'G13', 'G14', 'G16', 'G17', 'G25', 'G26', 'G33', 'G34']

const gejalaStep = computed(() => props.daftarGejala.filter(g => CODES.includes(g.kode)))

const toggle = (kode) => {
  let sel = [...props.modelValue]
  sel.includes(kode) ? (sel = sel.filter(k => k !== kode)) : sel.push(kode)
...
<template>
  <div class="animate-step">
    <div class="mb-9">
      <span class="text-[10px] font-black uppercase tracking-widest text-emerald-600">Langkah 4 dari 4</span>
      <h2 class="text-3xl font-black text-slate-800 tracking-tight mt-1 mb-2">Badan, Kulit & Kaki</h2>
      <p class="text-slate-500 text-sm max-w-xl leading-relaxed">Pemeriksaan integritas kulit, kondisi bulu, pencernaan lambung, dan kelainan cara berjalan atau kaki sapi.</p>
    </div>
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      >
        ...
        <div>
          ...
          <span class="text-[9px] uppercase font-bold tracking-widest text-slate-400 mt-1.5 block">Badan, Kaki & Kulit</span>
        </div>
      </label>
    </div>
  </div>
